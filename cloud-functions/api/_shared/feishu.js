// CloudBase 云函数：飞书 Open API 封装（CommonJS 版）
// 凭证来自环境变量 FEISHU_APP_ID / FEISHU_APP_SECRET，不落地存储。

const API = 'https://open.feishu.cn/open-apis';

function feishuCredentials(env) {
...
const defaultWikiNode = () => 'Rccuw5qndim133kO2TQcvHvcnSf';

function titleWithDate(title, created) {
...
async function appendBlocks(doc, auth, markdown, added) {
...
async function syncToFeishu(env, bytes, mime, filename, text, title, note) {
...
async function reviseFeishu(note, markdown, env) {
  const auth = await tenantToken(env);
...
module.exports = { feishu, tenantToken, syncToFeishu, reviseFeishu, markdownBlocks, segments, titleWithDate };
