// CloudBase 云函数：DeepSeek API（视觉识别 + 排版修改）
...
async function transcribe(bytes, mime, env) {
...
async function reviseMarkdown(markdown, instruction, env) {
  const apiKey = env.DEEPSEEK_API_KEY;
  if (!apiKey) throw new Error('请先配置 DeepSeek API Key（环境变量 DEEPSEEK_API_KEY）');
  return call(apiKey, env, [
...
module.exports = { transcribe, reviseMarkdown };
